...
export async function updateTodo(params: InsertTodos) {
	try {
		...
		const todo = await db
			.update(todos)
			.set({ updated_at: updatedAt, ...params })
			.where(eq(todos.id, params.id || 1))
			.returning();
		return todo;
	} catch (e) {
...
export async function getTodosByUpcoming(petId: number[], dateRange = new Date()) {
	try {
		return await db.query.todos.findMany({
			where: (todos, { gte, and, inArray }) =>
				and(gte(todos.dueDate.getSQL(), dateRange), inArray(todos.petId, petId)),
